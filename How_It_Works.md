...
Mentions have two halves: a compose-time autocomplete popup and a render-time pill with click-to-navigate.

**Compose path.** The input bar's keystroke handler runs `refreshMentionSuggest` after every update. When it detects a `@` at the start of input or right after whitespace (so emails like `me@example.com` don't trigger), it builds a candidate pool by merging Slack workspace users (`m.users`) with friends from the friend store, sorted alphabetically. Display names are deduped — if two entries collide (e.g. two friends both named "Alex"), the later ones get `-2`, `-3`, … suffixes so each pool entry is a unique string. Name precedence is *DM-channel alias > Slack DisplayName > Slack RealName > bare id* for Slack users, and the friend's `Name` field for friends. The popup filters by case-insensitive prefix as the user types after the `@`. Tab or Enter inserts the readable name (`@Ryan Weiss `) into the input, replacing the partial query. The wire-format conversion happens later, at send time: `resolveMentionsForSend` walks the input text, longest-name-first, and rewrites `@<name>` matches into `<@U12345>` (Slack) or `<@slacker:abc>` (friend) tokens. `@everyone` / `@here` / `@channel` and free-typed handles that don't match any pool entry are left untouched.

**Render path.** `format.FormatMessageWithMentions` is the entry point — it recognises both `<@U[A-Z0-9]+>` and `<@slacker:[A-Za-z0-9._\-]+>` mention forms, replaces each with a short `[MENTION:#m-N]` marker, and returns a parallel `[]Mention` slice with the resolved (id, name) tuples. The marker keeps the visual width predictable so word-wrap works the same for plain text and mentioned text. After wrapping, `rewriteMentionPills` walks each line, swaps markers for `@Name` rendered with the themed `MentionStyle`, and records the pill's column range as a `mentionHit` entry — same pattern as friend cards and reactions. A click on the pane checks `MentionAtClick(x, y)` first; on a hit, the model resolves the user id to a DM channel (Slack) or friend channel (P2P) and switches there.

**Theme integration.** A new `mention` theme key was added to `internal/theme` (in `AllKeys`, `KeyDescription`, the `Messages` key group, and `default.json`). Other built-in themes inherit via the existing `Get` fallback chain. Edit it via the in-app theme editor under Messages → mention.

**Friend resolver refresh.** A single `notifyFriendsChanged` model helper consolidates "rebuild the sidebar friend channels" and "re-seed the resolver map for FormatMessage" into one call, dispatched from every site that mutates the friend store (add / remove / rename / merge / replace). This keeps mid-session friend additions resolved without the user needing to restart.
