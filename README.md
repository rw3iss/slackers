...
- **Threads** -- any Slack channel or friend (P2P) chat where you're @mentioned, are the parent author, or have replied is auto-tracked as a thread. A `Threads` group surfaces at the top of the sidebar listing every active thread (two-row entry: source channel name on top, participants on the bottom). Friend-channel threads render in the same green as online friends so they're immediately distinguishable from Slack threads. Click or press Enter to open the parent message in the existing reply detail view; right-click for *Go to Channel* / *Close Thread*. Threads are persisted across restarts. An optional **auto-clear** sweep (Settings → Threads) removes inactive threads after 24h / 72h / 7d / 30d. A **backfill scope** setting controls how the global Threads view (coming soon) searches history for older threads — *disabled* / *local* (in-memory only, default) / *subscribed* (sidebar channels) / *all_public* (full workspace).
