...
- **Threads** -- any Slack channel or friend (P2P) chat where you're @mentioned, are the parent author, or have replied is auto-tracked as a thread. A `Threads` group surfaces at the top of the sidebar; each entry is two visual rows — alias / `#channel` / `@friend` with a right-aligned "time since last activity" badge (`5m`, `3h`, `2d`, …) on row 1, and the participants list on row 2. Friend-channel threads render in the online-friend green so they're immediately distinguishable from Slack threads. Click or press Enter to open the parent message in the existing reply detail view; right-click for *Go to Channel* / *Close Thread*. Threads persist across restarts. An optional **auto-clear** sweep (Settings → Threads) removes inactive threads after 24h / 72h / 7d / 30d. A **backfill scope** setting controls how the global Threads view (coming soon) searches history for older threads — *disabled* / *local* (in-memory only, default) / *subscribed* (sidebar channels) / *all_public* (full workspace).
- **@mentions** -- typing `@` in chat input opens an autocomplete popup of all Slack workspace users + friends, filtered by prefix as you type, navigated with arrows, completed with Tab/Enter. The readable display name (alias > display > real name) is inserted into the input; on send, names are resolved to Slack/friend wire format (`<@U…>` / `<@slacker:…>`) so notifications work upstream. Rendered mentions in chat are themed pills (configurable via the `mention` theme key) — clicking a pill opens that user's DM (Slack) or friend channel (P2P). Friend mentions handle dashed/underscored handles and are seeded into the resolver at boot, so friends-only mode resolves them too.
